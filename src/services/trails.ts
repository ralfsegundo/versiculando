...
export default function Trails({ onSelectTrail, onSelectBook }: TrailsProps) {
  const { profile, userId } = useGamification();
  ...
  // CORREÇÃO: Utilizando a fonte de verdade do Supabase para decidir se desbloqueia
  const showDeepJourney = profile.completedPlans >= 1;
  ...
  }, [profile.completedBooks, activeTab, userId]);
            ...
            {isDiscipleCompleted && !showDeepJourney && (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                ...
                <div className="relative z-10 flex flex-col items-center">
                  ...
                  <button onClick={() => { confetti({ particleCount: 100, spread: 70, origin: { y: 0.6 }, colors: ['#a855f7', '#6366f1', '#3b82f6'] }); }}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/70 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.85, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.85, y: 30 }} transition={{ type: 'spring', damping: 20, stiffness: 260 }} className="bg-white rounded-[2rem] max-w-md w-full shadow-2xl overflow-hidden">
              ...
              <div className="p-6 md:p-8">
                <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 mb-6 text-center"><p className="text-amber-800 font-bold text-lg">+1000 pontos conquistados! 🏆</p><p className="text-amber-600 text-sm mt-1">Uma conquista espiritual que ficará para sempre</p></div>
                ...
                <div className="space-y-3">
                  <button onClick={() => { setShowGraduationModal(false); setTimeout(() => confetti({ particleCount: 80, spread: 60, origin: { y: 0.7 }, colors: ['#a855f7', '#6366f1'] }), 200); }}
